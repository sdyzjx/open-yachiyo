...
OpenClaw 强在“高灵活、多渠道”的网关与编排能力。
`open-yachiyo` 优化的是另一条轴：**严格可控的运行时与确定性状态管理**。

| 维度 | OpenClaw（编排器） | open-yachiyo（运行时引擎） |
|---|---|---|
| 核心架构 | Prompt 驱动的路由/集成中枢 | 事件驱动的异步状态机 |
| 执行模型 | 线性编排流程 | 有界 ReAct 循环 + 精确步进控制 |
| 工具 I/O 链路 | 同步调用路径（按步阻塞） | EventBus 解耦 + early dispatch |
| 状态管理 | 相对松散的会话上下文 | 强会话隔离 + 显式持久记忆（SOP） |
| 产品定位 | 一套网关连接多聊天平台 | 高性能原生桌面 Agent 引擎 |

**结论：**
- 如果目标是快速把 LLM 能力路由到多个聊天平台，选 OpenClaw。
- 如果目标是构建复杂、客户端重交互的桌面 Agent（如 Live2D 助手），且要求严格恢复边界、可审计生命周期与确定性行为，选 open-yachiyo。
